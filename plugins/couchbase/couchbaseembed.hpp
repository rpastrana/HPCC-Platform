...
namespace couchbaseembed
{
    ...
    {
        //--------------------------SET----------------------------------------
        COUCHBASEEMBED_PLUGIN_API unsigned COUCHBASEEMBED_PLUGIN_CALL func1  (ICodeContext * _ctx, const char * param1, const char * param2, unsigned param3);
    ...
    {
    public:
        //inline CouchbaseConnection() : session(NULL) {}
        ...
        //inline CouchbaseConnection(CassSession *_session) : session(_session)
        //{
        //}
        inline ~CouchbaseConnection()
        {
        ...
        inline void connect()
        {
            ...
            if (error != LCB_SUCCESS)
                failx("Couldn't schedule connection, error: %d", error);

            lcb_wait(instance);
               error = lcb_get_bootstrap_status(instance);

               if (error != LCB_SUCCESS)
                   failx("Couldn't bootstrap from cluster, error: %d", error);
    ...
    class CouchbasePrepared : public CInterfaceOf<IInterface>
        {
        public:
            //inline CouchbasePrepared(const CassPrepared *_prepared, const char *_queryString)
