...
#endif
        //rodrigo
        //do we want the local span's client headers instead? Does it make any difference?
        //Owned<IProperties> traceHeaders = master->logctx.getClientHeaders();
        if (traceHeaders)
        {
            fprintf(stderr, "active span clientheader propogated: %s\n", traceHeaders->queryProp("traceparent"));
    ...
    void Do(unsigned idx)
    {
        ...
        while (!master->aborted)
        {
            ...
            for (;;)
            {
                ...
                catch (IException *e)
                {
                    ...
                    do 
                    {
                        ...
                        if (idx==startidx)
                        {
                            clientSpan->recordError("all_sockets_failed");
                            StringBuffer s;
            ...
            try
            {
                ...
                //clientSpan->setSpanAttribute("http.code", (int64_t)rval); future
                ...
                clientSpan->setSpanAttribute("http.response.status_code", "200");//rodrigo should be numeric
                ...
                clientSpan->recordSuccess("SoapCall Success");
                break;
            }
