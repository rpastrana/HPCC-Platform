...
interface IXmlWriterExt : extends IXmlWriter
{
    ...
    virtual void rewindToAndAttemptclose(unsigned int prevlen) = 0;//{ rewindTo(prevlen); }
...
public:
    ...
    virtual void rewindToAndAttemptclose(unsigned int prevlen)
    {
        rewindTo(prevlen);
        ...
        {
            tagClosed = false; // we can't assume the tag is now in closed mode
            closeTag();
        }
    }
...
public:
    ...
    virtual void rewindToAndAttemptclose(unsigned int prevlen)      { rewindTo(prevlen); }
