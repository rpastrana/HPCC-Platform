...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format"
  ...
  ================================================================================ -->
	...
	<!--xsl:param name="roxiehost" select="'aa'"/-->
	<!--xsl:param name="roxieport" select="'bb'"/-->
	<xsl:param name="servers" select="'cc'"/>
	<!-- ===============================================================================-->
	<xsl:template match="/">
	<html>
		<head>
...
{
	//expecting <action><content>XML</content></action>
	try
  	{
  		//IE
...
function onSendRequest()
{
	...
	if (document.getElementById("req_host").value == "") // || document.getElementById("req_port").value == "")
	{
		alert("Please speficy the target ROXIE host and port in the 'Advanced' section");
		return true;
	}
   ...
   // document.getElementById("resp_header").value = "";
	...
	url = url + "&roxie-host=" + document.getElementById("req_host").value;
	url = url + "&roxie-port=" +  document.getElementById("req_port").value;
	//url = url + "&roxie-port2=" +  document.getElementById("req_port2").value;
...
{
	var ctrl = document.getElementById("servers_list");
	var protocol = ctrl.options[ctrl.selectedIndex].getAttribute("protocol");
	var host = ctrl.options[ctrl.selectedIndex].getAttribute("host");
	var port = ctrl.options[ctrl.selectedIndex].getAttribute("port");

	document.getElementById("req_host").value = host;
	document.getElementById("req_port").value = port;
	//document.getElementById("req_port2").value = port;
						   ...
						   <table width="100%">
							<tr> <td align="left"><b>Request Body:</b></td> 
							 <td align="right">
							       ...
							       <xsl:if test="$inhouseUser"><xsl:text> </xsl:text><input id="import" type="button" value="Import..." onclick="onImportConciseRequest()"/></xsl:if>
					...
					<tr>
						<td align="left" class='options'>
							...
							<span id='option_span' style='display:none'>
								...
								<td>
									<select id="servers_list" onchange="updateServers()">
									  <xsl:value-of select="$servers" disable-output-escaping="yes"/>
									</select>
								</td>
							</tr>
									<tr>
									...
									<tr>
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="1">
											...
											<xsl:element name="input">
												...
												<xsl:attribute name="id">req_port</xsl:attribute>
												<xsl:attribute name="size">5</xsl:attribute>
											</xsl:element>
											<!-- 
											<xsl:element name="input">
												<xsl:attribute name="type">text</xsl:attribute>
												<xsl:attribute name="id">req_port2</xsl:attribute>
												<xsl:attribute name="size">5</xsl:attribute>
											</xsl:element-->
											</tr>
										</table>
											<!--input type="text" id="req_port"-->
											<!--xsl:value-of select="$roxieport" /-->
											<!--/input-->
